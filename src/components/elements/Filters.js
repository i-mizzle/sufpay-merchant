...
const Filters = ({filterOptions, returnSelected, resetFilters, checkConditions, performFilterSearch}) => {
    ...
    return (
        <div className="flex flex-row gap-2 flex-wrap">
            <span className={`py-2 px-3 rounded-md text-gray-600 font-poppins text-xs flex items-center justify-center gap-x-1 ${activeFilters && activeFilters.length > 0 ? 'bg-vcm-light-purple bg-opacity-20' : 'bg-gray-100'}`}> 
                <FunnelIcon className="w-4 h-4 inline" /> Filters
            </span>

            { activeFilters.map((filter, filterIndex) => (
                <span className='py-2 px-3 rounded-md bg-gray-100 text-gray-600 flex gap-x-3 items-center text-xs font-poppins' key={filterIndex}> 
                    {filter.displayValue}
                ...
                <div className={`z-50 bg-white rounded-md shadow-lg p-2 w-44 absolute top-0 left-14 ${ selectingFilters ? 'inline-block' : 'hidden' }`}>
                    <div className='relative'>
                        {filterOptions.map(( option, optionIndex ) => (
                            optionIndex !== 0 &&
                            <button key={optionIndex} className={`capitalize text-xs text-left my-1 p-2 hover:bg-gray-100 rounded-sm transition duration-200 text-gray-600 w-full block ${optionIndex === activeFilterOption ? 'bg-gray-100' : ''}`} onClick={()=>{setActiveFilterOption(optionIndex)}}>
                                {option.name}
                            </button>
                        ))}
                        {activeFilterOption && activeFilterOption !== 0 && 
                            <div className='absolute top-0 left-44 w-44 rounded-sm p-5 bg-white shadow-lg'>
                                <p className="text-xs font-poppins text-gray-600 mb-2 rounded-sm capitalize">{filterOptions[activeFilterOption].name}</p>
                                {filterOptions[activeFilterOption].linkType === 'text' && <p className="text-xs text-gray-600">{filterOptions[activeFilterOption].link}</p>}
                                {filterOptions[activeFilterOption].linkType === 'option' && 
                                    <select className="text-xs text-gray-600 p-1 border rounded-sm w-full my-2 outline-none" onChange={(e)=>{setFilterLinkOption(e.target.value)}}>
                                ...
                                {filterOptions[activeFilterOption].type === 'binary' && 
                                    <select className="text-xs text-gray-600 p-1 border rounded-sm w-full my-2 outline-none" onChange={(e)=>{setFilterValue(e.target.value)}}>
                                        <option value="">-- select one --</option>
                                ...
                                {filterOptions[activeFilterOption].type === 'number' && 
                                    <input placeholder='value' type='number' className="text-xs text-gray-600 p-1 border rounded-sm w-full my-2 outline-none"  onChange={(e)=>{setFilterValue(e.target.value)}} />
                                }

                                {filterOptions[activeFilterOption].type === 'free-text' && 
                                    <input placeholder='value' type="text" className="text-xs text-gray-600 p-1 border rounded-sm w-full my-2 outline-none"  onChange={(e)=>{setFilterValue(e.target.value)}} />
                                }

                                {filterOptions[activeFilterOption].type === 'search' && 
                                    <input placeholder='search' onChange={(e)=>{performSearch(e.target.value)}} type="text" className="text-xs text-gray-600 p-1 border rounded-sm w-full my-2 outline-none" />
                                }

                                <button className='w-full p-1 rounded-sm text-white text-xs bg-gray-600 my-3 transition duration-200 hover:bg-gray-800' onClick={()=>{addFilter(activeFilterOption)}}>Add Filter</button>
                            </div>
                        }
                    </div>
                </div>
